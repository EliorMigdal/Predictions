...
public class EnvironmentPropertyRequest implements Request {
    public PropertyResponseDTO getEnvironmentProperty(String simulationName, String propertyName) {
        try {
            HttpRequest httpGet = new Get();
            CloseableHttpResponse response = httpGet.getResponse("/GetEnvironmentProperty",
                    new EnvironmentPropertyRequestDTO(simulationName, propertyName));
